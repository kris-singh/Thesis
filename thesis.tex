...
Stochastic Gradient Descent(SGD) methods are one of the main reasons for the success of Deep Learning. With the availability of "Big-Data" stochastic methods have proven to be the most effective methods for training Deep Neural Networks, as training on the full dataset is infeasible. \\\\
In machine learning we are interested in minimizing the loss function which is generally of the form,
\begin{equation*}
...
The training data is represented as ${x_i, y_i}$ where N denotes the total number of data points.
A typical way of solving these optimization problems is through gradient descent algorithm
\begin{align}
\begin{split}
    \theta_{t+1}&=\theta_{t} - \eta \frac{1}{N}\sum_{i=1}^N \nabla( ; \theta_t)\\
      ...
      \end{split}
\end{align}
The problem with above formulation is that we are required to calculate the gradients of each of the N data points, which is prohibitively expensive even for a small N. SGD provides a solution to above problem where the parameter update is conditioned upon only a  single  data point selected uniformly as random
\begin{align}
\begin{split}
...
It can be seen that $\nabla(x_i, \theta_t)$ is an unbiased estimate of  full batch gradient $\nabla (:, \theta_t)$ as  $\E (\nabla( x_i, \theta_t)) = \frac{1}{N}\sum_{i=1}^N \nabla f(x_i; \theta_t)$ but, SGD can introduce high variance between the estimated gradient $\nabla(x_i, \theta_t)$ and the true gradient $\nabla(; \theta_t)$ , due to this reason stochastic gradients have a much slower convergence to the optimal parameters. A simple way of reducing the variance is to use mini-batch SGD. 
\begin{align}
\begin{split}
...
Another way of reducing the variance induced by stochastic sampling is to sample data points that are representative of the full data set.  Higher importance should be given to data points that provide the most information to the network, in literature \cite{shrivastava2016training} this called as hard example mining. Choosing the data points in this way is called Importance Sampling.  % * <vineethnb@iith.ac.in> 2018-03-04T13:38:10.150Z:
% 
% > \begin{align*}
...
At present we approximate the full gradient using a Monte Carlo estimate given by
\begin{align}
\tilde{\nabla}(f(; \theta_t)) & = \frac{1}{B}\sum_{i=1}^B p(x_i) \nabla(f(x_i; \theta_t))\\
& = E_{p(x)}\nabla(f(x_i; \theta_t)
\end{align}
Here p(x) denotes the sampling distribution (Uniform) and that we use  for selecting B data points ie , $P(x=x_i) = p(x=x_i) = \frac{1}{B}$ \\
...
\begin{split}
 ...
 \end{split}
\end{align}
Here q(x) denotes the surrogate sampling distribution and that we use  for selecting B data points ie , $P(x=x_i) = q(x=x_i)$ \\
In later section we show that if q(x) chosen in appropriately, then we get an unbiased estimator. Also we prove that using importance sampling leads to variance reduction. It is important to note that q(x) is just a dummy variable.
...
Some known strategies for importance sampling for SGD are to sample data points proportional to the loss, gradient wrt to current parameters for the model etc.  Sampling with respect to gradients essential is an case of hard example mining. \\
But, the drawback with gradient based sampling is that for sampling it requires the full gradient and it is sensitive to outliers. Moreover, mini-batches should consist of not only the most informative data points but also points that are diverse enough to represent the full data set for eg include data points from all classes rather than just one class. This becomes much more important in the case of imbalanced datasets which require that sampling data points for classes such that we get more balanced minibatches.
\\
\section{Diversity}
The diversity of mini-batch is an important quality for circumventing the problem of bias in mini-batches. A popular method for modeling diversity in literature is to use Submodular Functions. A submodular functions is defined as follows,\\
A function $f:2^V\longleftarrow R$ is sub modular if for any A$\subseteq$ B $\subseteq$ V, and v $\in$ V/B,we have that:
    \begin{equation*}
        f(A\cup v) - f(A)\geq f(B\cup v)- f(B)    
    \end{equation*}
using the definition we can  see that submodular functions  are good candidates for modeling diversity. The above property is called the diminishing return property of the submodular functions. Submodular function are ideally suited for selection of a diverse set of data points as, these functions are easy to maximize as they just require the most greedy solution at each step which makes their optimization fast. 
...
The DPP assigns a probability of sub sampling any subset Y $\subseteq \{1,...,N\}$, which is proportional to the determinant of the sub-matrix $\mathbf{L_Y}$ of   $\mathbf{L}$ which indexes the subset,
...
Thus, a need arises for models that model diversity as well as DPP but, are much easier to sample from.  Another, problem with DPP is defining the similarity kernel for datasets which can be hard to find for more complex datasets. 
\\
\\
The main contributions of our work are the following 
\begin{itemize}
    \item Design a sampling scheme that encodes both importance of sample and also sample diversity for the creation of representative batch for the whole data set.
    \item Use of submodular objective function for assigning weights to sample. This enables us to sample points in a greedy manner which is much better than existing DPP technique sampling.
    ...
    \item We carry out extensive experiments on several models and datasets. Our approach leads to faster learning and higher classification accuracy in image classification task. 
\end{itemize}

As far as we know our work is one of the first to use submodular functions for importance sampling in the context of variance reduction in SGD. Also, we are one of first papers that perform large-scale image classification task on datasets such as cifar-10 and MIT-67 datasets.

\part{Prerequisites }

\chapter{Deep Neural Networks and Stochastic Gradient Descent}
In this chapter we first give a brief overview of Neural Networks and Deep Learning. We then derive the Backpropgation algorithm and show how SGD(Stochastic Gradient Descent) Algorithm is used to train the network.
\\\\
Machine learning is commonly divided into two main types:
\begin{itemize}
\item Supervised Learning: In this type of learning we are trying to learn a hypothesis $\mathcal{H} : X \rightarrow y$ which maps from the input data domain to the outputs (or training signal). If the output set Y is finite, we say that the task is a classification task and if it is continuous we say that it is a regression task.
...
Deep Learning has revolutionized the fields of both supervised 
and Unsupervised Learning. Tasks which once seemed impossible to be ever done by a machine are now being excelled at by machines. ImageNet Classification challenge which sparked the boom for the Deep Learning Research, is a solved problem now with the machine learning  achieving super human level of accuracies on the task. \\ \\
There still exist a few problems with Deep Learning such as, 
\begin{itemize}
\item Dataset: Deep Learning Networks require huge amounts of Data to trained. It is not always possible to have acess to such data. There is a need for algorithms that train faster and better using only a small subset of data.
...
\end{itemize}
Our work tries to mitigate both of the above points through importance sampling for training of Deep Neural Networks.

In this section we derive the back propagation algorithm used to train neural networks. \\
...
\end{figure}
We take a Neural Network (NN) with the following parameters 
\begin{itemize} 
...
We can think of $w_{ki}$ as the k,i entry in the matrix $\mathbf{W}^{K \times N}$, similarly $w_{ij}'$ can be considered as the i,j entry in the $\mathbf{W}^{K \times N}$,. The activation function f is equal the sigmoid activation function.
\\ 
\\
\textbf{Forward Pass}
In the forward pass we pass a single data point through the NN.
\begin{align}
...
&\text{We assume a Mean Squared Error Loss} \\
...
This is the famous Backpropogation algorithm it is called so, because we are back propagating the gradients from output to the input.\\
...
\end{align}
Here f denotes the Loss function and $\textbf{x}_i$ denotes the i th point in the dataset $\mathcal{D}$ of size N . We can see that computing the gradient  for all of the N data points is computationally expensive. In practice only a part of dataset is used for computing the gradient and  updating of the parameters. \\
\subsection{Stochastic Gradient Descent (SGD)}
The stochastic gradient estimates the true gradient on the whole dataset by using the gradient of only a single data point. The update rule for the SGD algorithm is given by 
\begin{align}
...
\end{align}
This show that the stochastic gradient estimate is unbiased but, the variance of stochastic gradient algorithm is non-zero as can be seen below , \\
...
\end{align}
Where , $\mathbb{V}(\nabla f(x; \theta)) = \E || \nabla f(x; \theta) - \E\nabla f(x; \theta)||^2 $.\\\\
For the above Eq \ref{Variance Red} the smaller the variance the the more the reduction on the objective function we get. \\ In the next chapter we see some popular techniques in literature that have been used for variance reduction.
...
\label{Greedy}
\begin{algorithmic}[1]
\Require{A set V, A set function $f : 2^V \rightarrow \mathbb{R}$, Size of subset k.}
\Ensure{A subset $S_k \subset V$ of size k}
\State $S_0= \phi$ 
\For{$i=0$ to $k$}
...
In this chapter we give a brief survey of  the works that use optimization techniques to solve the variance reduction problem is SGD.\\

As stated earlier one of the major disadvantages of using mini batch SGD is that the randomness due to sampling of data points can induce a large variance between the true gradient and the mini-batch gradient leading to slower convergence of SGD. Many variance reduction techniques have been introduced to tackle this issue. 
\\
...
\end{split}
\end{align}
For reduction in variance we can either change how we create mini-batches(importance sampling), change learning rate($\eta$), change batch size($b$) or change the way we updates the parameters. We discuss how some works have used these to obtain variance reduction in SGD.
\\ 
\section{Non-Sampling Based}
These techniques are the one's that sample the data points using a uniform distribution. They can further be classified into the following\\
\textbf{Averaging Techniques}: These techniques try to reduce variance using averaging of parameter \cite{polyak1992acceleration} or averaging of the gradients \cite{nesterov2013introductory}. Averaging based methods  try to get smooth approximation of gradients or parameters, thus reducing the variance. 
\\
If we look at Eq \ref{SGD}
\begin{equation}
\theta_{t+1} = \theta_{t} - \eta \nabla(f(x_i; \theta_t)) 
\end{equation}
and change the update rule to compute the gradient over a minibatch of data points $B = \{x_j \in \mathcal{D} \space| \space j={1, 2, ..., b}\}$ as follows 
\begin{equation} \label{MSGD}
\theta_{t+1} = \theta_{t} - \frac{\eta}{b} \sum_{i=1}^B \nabla(f(x_i; \theta_t))  
\end{equation}

Where b denotes the size of a uniformly random sampled batch from full dataset $\mathcal{D}$. This called as Minibatch Gradient Descent. \\
Minibatch Gradient Descent leads to reduction in the variance Changing Eq \ref{Variance Red} to, 

\begin{equation}
\E f(x; \theta_{t+1}) \leq  f(x; \theta_t) - \eta(1 -\frac{L\eta}{2}) ||\E \nabla f(; \theta_{t})||^2 + \frac{L\eta^2}{2 b}  \mathbb{V}(f(x ;\theta) 
\end{equation}
Here we see that the variance reduces by factor b.  Though, the computation cost increase by b, but in general we require minibatch gradient descent for better generalization error in lesser time and also for parallelization of SGD.
\\
\\
\textbf{Momentum} based techniques try to use momentum from past gradients to choose the descent direction\cite{johnson2013accelerating} \cite{schmidt2013minimizing} . The descent direction is based on the averaged descent direction which are updated periodically. 
The simplest momentum based method uses the following update rule 
\begin{equation}
\theta_{t+1} = \gamma \theta_{t} - \eta \nabla(f(x); \theta_t)) 
\end{equation}
$\gamma \leq 1$ parameter. Momentum based methods try to reduce the oscillation in gradient descent algorithm. It does by adding a damping factor $\gamma$ if a direction for optimization when the gradient for a dimension changes drastically. As a result, we gain faster convergence and reduced oscillation. When momentum method is combined with minibatch sgd  \ref{Variance Red} equation changes to 
\begin{equation}
...
\\
\\
\textbf{Adaptive Learning Rates and Batch Size}: Setting adaptive learning rates based on local curvature\cite{schaul2013no} or based on gradient information \cite{zeiler2012adadelta} has been shown to reduce variance. We show a particular case for AdaGrad Method, the update rule changes to 
\begin{equation}
	\theta_{t+1} = \gamma \theta_{t} - \frac{\eta} {\epsilon + G_t} \nabla(f(\textbf{x}; \theta_t)) 
\end{equation}
Where $G_{t}^{D\times D}$,  G here is a diagonal matrix where each diagonal element i, is the sum of the squares of the gradients w.r.t. $\theta^i_t$ (i th dimension of $\theta$) up to time step t , while $\epsilon$ is a smoothing term that avoids division by zero. It adapts the learning rate to the parameters, performing smaller updates 
...
Other papers have explored the coupling of learning rate and batching size. These methods try to optimizes for adaptive batch size for reduction of variance. Some methods \cite{balles2016coupling} try to adapt batch size based on the variance of the gradient estimates others \cite{devarakonda2017adabatch} adaptively increases the batch size during the training process slower initially and then doubling the batch sizes at specified intervals.\\

\chapter{Importance Sampling through Optimization }
We now discuss some importance sampling based methods. As discussed earlier in case of importance sampling\ref{Imporatnce Sampling}  instead to sampling from the defined distribution p(x) we try to find another distribution h(x). The tricky part is finding a distribution h(x) which is easy to sample from as well as it reduces the variance of the approximated quantity.
\section{Online Learning To  Sample \cite{bouchard2015accelerating}}
Remember the variance of importance sampling is given by Eq \ref{Variance Importance Sampling} 
\begin{equation}
...
\end{equation}
if we are sure that $f(x) \in H(x; \tau)$ then, there exist a $\tau^*$ such that $h(x; \tau^*) \propto f(x)$. In general this is not true and also finding a closed form solution for the min $var_\tau(\tilde\theta)$ is not possible. Hence we need to find an approximate variance reduction methods.  Taking the gradient wrt $\tau$  we get, 
\begin{align}
...
\end{align}
 We can update a value for $\tau$ using the SGD update rule, here $x_t \sim h(x_t; \tau)$
 \begin{equation}
 \tau_{t+1} = \tau_{t} - \eta \bigg(\Big(\frac{f(x_t)}{h(x_t;\tau)}\Big)^2 \nabla_\tau\log h(x_t; \tau) \bigg)
 \end{equation}
The paper shows good results on many deep learning tasks like Image in painting , matrix factorization etc. \\ A major problem with evaluation schemes seems to be that the evaluation is based on speed up vs the number of epochs rather than speed up vs time, since finding the value of $\tau$ itself is computationally intensive process the speed vs time makes more sense.

\section{Adaptive Sampling for SGD by Exploiting Side Information\cite{gopal2016adaptive}}

This works tries to find an the distribution q(x) using additional information from the problem which it calls as the side information. This side information can be anything like labels etc. The papers uses this latent information for maintaining probability distribution over the bins constructed using the side information. The optimal sampling distribution corresponds to sampling more instances from those bins which have larger gradients.  First the samples are distributed into binds according to their side information, $C_k$ denotes set of all training instances which have the side information set k. The papers proposes the following update rule 
\begin{equation}
\theta_{t+1} = \theta_{t} - \frac{1}{N} \frac{1}{P(i)}\nabla f(i; \theta_{t}) \\ 
\end{equation}
Where  $P(i) = \frac{p_k}{C_k}$ , this can be though of as a step process where you firstly sample the cluster uniformly at random and the sample from the cluster according to the distribution $p_k$. The paper shows that given this side information we can solve an optimization problem  for the maximum reduction in variance which gives the optimal distribution $p_k$ as ,
\begin{equation} \label{gopal}
p_k \propto \frac{|C_k|}{N} \sqrt[]{\frac{1}{|C|_k} \sum_{i \in C_k } || \nabla f(i; \theta_t} || ^2
\end{equation}
$p_k$ denotes the probability of selecting a data point inside cluster/bin k. We can easily see that sampling scheme is unbiased.\\
The paper uses a simple idea of first sampling a bin and then selecting a data point using Eq \ref{gopal} for selecting a point from the bin. This is a very simple idea and achieves great results on tasks of Logistic Regression but doesn't explore the idea to deep learning settings  as the  computation time for calculating the gradient is quite large. Also paper is not very clear about what other side information can be used other than labels.
